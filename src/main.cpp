...
void info(const SearchResult& r) {
    std::ostringstream o; o<<"info depth "<<r.depth<<" score "<<score_text(r.score)<<" nodes "<<r.nodes<<" time "<<r.elapsed_ms<<" nps "<<(r.nodes*1000/std::max<long long>(1,r.elapsed_ms))<<" pv";
...
void uci() {
    ...
    std::string syzygy; int proof_plies=5,threads=1; std::uint64_t proof_nodes=15000; Features features; bool selective=true;
    ...
    while(std::getline(std::cin,line)) {
        ...
        try {
            if(command=="uci") {
                emit("id name Axiom Chess 0.6-attribution\nid author Axiom contributors\noption name Hash type spin default 32 min 1 max 1024\noption name Threads type spin default 1 min 1 max 16\noption name Experimental type check default false\noption name Middlegame type check default false\noption name ProofPlies type spin default 5 min 0 max 15\noption name ProofNodes type spin default 15000 min 0 max 10000000\noption name SyzygyPath type string default <empty>");
                ...
                while(std::getline(names,name,',')) emit("option name Feature_"+name+" type check default "+(name=="pawn_cache"?"true":"false"));
                emit("uciok");
            }
                ...
                while(in>>token) {
                    if(token=="depth") { in>>limits.depth; explicit_limit=true; }
                    else if(token=="movetime") { in>>limits.milliseconds; limits.milliseconds=std::max(1,limits.milliseconds); explicit_limit=true; }
                    else if(token=="nodes") { in>>limits.nodes; explicit_limit=true; }
                    else if(token=="wtime") in>>wtime; else if(token=="btime") in>>btime;
                    else if(token=="winc") in>>winc; else if(token=="binc") in>>binc; else if(token=="movestogo") in>>movestogo;
                    else if(token=="infinite") infinite=true; else if(token=="verify") limits.verify=true;
                    else if(token=="mate") { int n=0; in>>n; limits.mate_plies=std::clamp(2*n-1,1,25); limits.proof_nodes=1000000; limits.depth=std::max(1,limits.mate_plies); explicit_limit=true; }
                ...
                if(remaining>=0 && limits.milliseconds==0) {
                    ...
                    int allocation=std::max(1,std::min(available,remaining/std::max(1,movestogo)+increment*3/4));
...
int main(int argc,char** argv) {
    try {
        ...
        Board board(fen);
        if(mode=="--perft") { if(perft_depth<0 || perft_depth>8) throw std::invalid_argument("perft depth must be 0..8"); std::cout<<perft(board,perft_depth)<<'\n'; }

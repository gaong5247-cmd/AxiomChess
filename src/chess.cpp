...
Undo Board::push_null() { Undo u{squares,side,castle,ep,halfmove,fullmove,history.size()}; side=-side; ep=-1; return u; }
void Board::pop(const Undo& u) { squares=u.squares; side=u.side; castle=u.castle; ep=u.ep; halfmove=u.halfmove; fullmove=u.fullmove; history.resize(u.history_size); identities.resize(u.history_size); }
std::vector<Move> Board::legal_moves() {
    ...
    for(auto m:moves) { bool kingMove=std::abs(squares[m.from])==King; auto u=push(m,false); bool ok=!attacked(kingMove?m.to:king,side); pop(u); if(ok) legal.push_back(m); } return legal;
}
...
bool Board::gives_check(Move m) { auto u=push(m,false); bool check=in_check(); pop(u); return check; }

...
void Search::order(Board& b,std::vector<Move>& moves,Move tt,int ply) {
    ...
    for(auto m:moves) {
        ...
        ranked[ordinal]={score,ordinal,m}; ++ordinal;
    }
    ...
    for(std::size_t i=0;i<moves.size();++i) moves[i]=ranked[i].move;
}
int Search::quiescence(Board& b,int alpha,int beta,int ply,int qply,bool synthetic,bool clean) {
    ...
    for(auto m:moves) {
        ...
        if(limits_.selective && !clean && !check && cap && !checking && !m.promotion && !protected_capture) {
            ...
            int exchange=see(b,m); AXIOM_TRACE_DETAIL(causal.base.see=exchange);
...
int Search::negamax(Board& b,int depth,int alpha,int beta,int ply,bool pv,bool null_allowed,std::vector<Move>& line,bool synthetic,Move excluded,bool auxiliary,int extensions,bool cut_node) {
    ...
    tick(); auto moves=[&] { ProfileScope timer(limits_.profile,stats_.legal_ns); return b.legal_moves(); }(); bool check=b.in_check();
    if(moves.empty()) return check?-MateScore+ply:0;
    if(synthetic?b.insufficient():b.automatic_draw()) return 0;
    bool claim=!synthetic && b.can_claim_draw(true);
    ...
    for(auto m:moves) {
        ...
        if(can_skip && f.see_pruning && depth<=3 && see(b,m)<-(cap?90:40)*depth*depth) { ++stats_.see_prunes; AXIOM_TRACE_EVENT("SEE_PRUNE_EXECUTED",m,std::nullopt); continue; }
        int reduction=0;
